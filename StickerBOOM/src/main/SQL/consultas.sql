...
-- FIGURINHAS DE ALGUMA COPA VENDIDAS PELA BANCA LOCALIZADA NA RUA  'Rua Thiago Silva, 3'
SELECT BF.ALBUM, BF.IDENTIFICADOR, COUNT(*) FROM BANCA_FIGURINHA BF
JOIN BANCA B
ON BF.BANCA = B.CNPJ
JOIN ALBUM A
ON BF.ALBUM = A.ISBN
...
-- RETORNAR AS AVALIACOES DE NEGOCIACAO DE VENDA ORDENADAS POR NOTA
SELECT A.NOTA, A.COMENTARIO
FROM AVALIACAO A
JOIN AVALIACAO_VENDA AV
ON A.COLECIONADOR = AV.COLECIONADOR_1 AND A.DATA_HORA = AV.DATA_HORA
ORDER BY NOTA;

-- DIVISAO RELACIONAL: PEGAR TODOS OS COLECIONADORES TODOS OS ALBUNS CADASTRADOS NO SISTEMA

-- R = ALBUM_VIRTUAL
-- S = ALBUM
-- y = ALBUM.ISBN
-- x = ALBUM_VIRTUAL.COLECIONADOR

-- SELECT DISTINCT AV1.COLECIONADOR
-- FROM ALBUM_VIRTUAL AV1
-- WHERE NOT EXISTS ((SELECT ISBN FROM ALBUM)
--     EXCEPT (SELECT AV2.ALBUM FROM ALBUM_VIRTUAL AV2
